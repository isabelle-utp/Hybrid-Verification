...
thm c1_implies_local_lipschitz continuous_on_iff bdd_linear_iff_has_derivative
lemma c1_local_lipschitz: 
  fixes f::"real \<Rightarrow> ('a::{banach,perfect_space}) \<Rightarrow> 'a"
    and f' :: "real \<times> 'a \<Rightarrow> 'a \<Rightarrow> 'a"
  assumes "open S" and "open T" and bdd_linear: "\<forall>\<tau> \<in> T. \<forall>s \<in> S. bounded_linear (f' (\<tau>, s))"
    and c1hyp: "\<forall>\<tau> \<in> T. \<forall>s \<in> S. D (f \<tau>) \<mapsto> (f' (\<tau>, s)) (at s within S)" "continuous_on (T \<times> S) f'"
  shows "local_lipschitz T S f"
proof(unfold local_lipschitz_def lipschitz_on_def, clarsimp simp: dist_norm)
  {fix \<tau> and s
    assume "s \<in> S" and "\<tau> \<in> T" 
    then obtain L where bdd: "\<forall>x. \<parallel>f' (\<tau>, s) x\<parallel> \<le> L * \<parallel>x\<parallel>"
      using c1hyp that
      unfolding has_derivative_def bounded_linear_def bounded_linear_axioms_def
      by (auto simp: mult.commute)
    hence "L \<ge> 0"
      by (metis mult.commute mult.left_neutral norm_ge_zero 
        order_trans vector_choose_size zero_le_one)
    moreover have "continuous_on UNIV (f' (\<tau>, s))"
      apply (rule_tac f'="\<lambda>t. f' (\<tau>, s)" in has_derivative_continuous_on)
      subgoal for x
        using bdd_linear[rule_format, OF \<open>\<tau> \<in> T\<close> \<open>s \<in> S\<close>] 
        by (subst (asm) bdd_linear_iff_has_derivative[where F="at x"], force)
      done
    ultimately have "continuous_on UNIV (f' (\<tau>, s)) \<and> (\<exists>L\<ge>0. \<forall>x. \<parallel>f' (\<tau>, s) x\<parallel> \<le> L * \<parallel>x\<parallel>)"
      using bdd by blast
  }
  note pair_L_bound = this[rule_format]
  ...
  {fix \<tau> assume tau_hyp: "\<tau> \<in> ?B1 \<inter> T"
    {fix x and y assume x_hyp: "x \<in> ?B2 \<inter> S" and y_hyp: "y \<in> ?B2 \<inter> S"
      ...
      hence "\<forall>r\<in>{0..1}. D f \<tau> \<mapsto> f' (r, s) at (\<sigma> r) within \<sigma> ` {0..1}"
      ...
      hence "\<And>r. r \<in> {0..1} \<Longrightarrow> (f \<tau> \<circ> \<sigma> has_vector_derivative \<DD> (y - x)) (at r within {0..1})"
        ...
        unfolding has_vderiv_on_def sorry by blast
