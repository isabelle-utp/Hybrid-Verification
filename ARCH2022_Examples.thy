...
lemma "A \<ge> 0 \<Longrightarrow> b > 0 \<Longrightarrow> (\<lambda>s::real^4. s$2^2 \<le> 2*b*(m-s$1) \<and> s$2 \<ge> 0) \<le>
  ...
  apply (auto simp: power2_diff[symmetric] algebra_simps(20,21) 
      mult.assoc[symmetric] power_mult_distrib power2_eq_square)
  sorry (* need to find correct algebra simps *)
