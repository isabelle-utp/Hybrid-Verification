...
definition fdia :: "('a \<Rightarrow> 'b set) \<Rightarrow> 'b pred \<Rightarrow> 'a pred" ("|_\<rangle> _" [61,81] 82)
  where "|F\<rangle> P = (\<lambda>s. (\<exists>s'. s' \<in> F s \<and> P s'))"

\<comment> \<open> Skip \<close>

abbreviation "skip \<equiv> (\<lambda>s. {s})"

lemma fbox_eta[simp]: "|skip] P = P"
